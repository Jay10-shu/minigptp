...
class BaseDatasetBuilder:
    ...
    #         if split not in ["train", "validation", "test"]: #cambiato val in validation
    #             continue
    ...
    #             ann_paths=ann_paths[0],
    #             vis_root=vis_path,
    #         )

    #     return datasets
    # In base_dataset_builder.py, replace the ENTIRE build function with this:
    def build(self):
        self.build_processors()
        ...
        for split in ann_info.keys():
            ...
            # annotation path (local variable)
            ann_paths = ann_info.get(split).storage
            if isinstance(ann_paths, str):
                ann_paths = [ann_paths]
            ...
            # create datasets - THIS IS THE KEY FIX
            dataset_cls = self.train_dataset_cls if is_train else self.eval_dataset_cls
            datasets[split] = dataset_cls(
                vis_processor=vis_processor,
                text_processor=text_processor,
                vis_root=vis_path,
                ann_path=ann_paths[0] # <-- Ensure this argument is passed correctly
            )
